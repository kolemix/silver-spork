...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .preview-img { border-radius: 10px; border: 2px solid #e74c3c; margin-top: 10px; max-height: 150px; }
...
<div class="container mt-4">
    ...
    <div class="form-card">
        <form method="POST" action="/admin/sach/sua/{{ $sach->id }}">
            @csrf
            <div class="form-group">
                <label>📖 Tên sách</label>
                <input type="text" name="tieu_de" class="form-control" value="{{ $sach->tieu_de }}" required>
            </div>
            <div class="form-group">
                <label>✍️ Tác giả</label>
                <input type="text" name="tac_gia" class="form-control" value="{{ $sach->tac_gia }}">
            </div>
            <div class="form-group">
                <label>🏢 Nhà xuất bản</label>
                <input type="text" name="nha_xuat_ban" class="form-control" value="{{ $sach->nha_xuat_ban }}">
            </div>
            <div class="form-group">
                <label>🏪 Nhà cung cấp</label>
                <input type="text" name="nha_cung_cap" class="form-control" value="{{ $sach->nha_cung_cap }}">
            </div>
            <div class="form-group">
                <label>📦 Hình thức bìa</label>
                <input type="text" name="hinh_thuc_bia" class="form-control" value="{{ $sach->hinh_thuc_bia }}">
            </div>
            <div class="form-group">
                <label>📝 Mô tả</label>
                <textarea name="mo_ta" class="form-control" rows="3">{{ $sach->mo_ta }}</textarea>
            </div>
            <div class="form-group">
                <label>🖼️ Link ảnh bìa</label>
                <input type="text" name="link_anh_bia" class="form-control" value="{{ $sach->link_anh_bia }}" id="linkAnh">
                <img src="{{ $sach->link_anh_bia }}" class="preview-img" id="previewAnh">
            </div>
            <div class="form-group">
                <label>💰 Giá bán</label>
                <input type="number" name="gia_ban" class="form-control" value="{{ $sach->gia_ban }}" required>
            </div>
            <div class="form-group">
...
<script>
// Preview ảnh khi đổi link
document.getElementById('linkAnh').addEventListener('input', function() {
    document.getElementById('previewAnh').src = this.value;
});
</script>
</body>
</html>